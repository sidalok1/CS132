...
public class Parser {
	...
	/**
	 * Parse table for an LL(1) grammar
	 * <p>
	 *     This table is implemented as an EnumMap of EnumMaps of symbol lists.
	 *     More specifically, the outer map takes the nonterminal and returns
	 *     the inner map, which returns a list of symbols representing the
	 *     production rule that should be selected.
	 *     See the static block for the Parser class for implementation
	 * </p>
	 * @see Parser
	 * @see Symbol
	 */
	...
	/**
	 * Constructor for the Parser class. Takes as input a queue of tokens and
	 * automaticaly parses them, creating a parse tree.
	 * @see Parser#parseTable
	 * @see Parser#parse(Node)
	 * @param tokens Tokens generated by the lexer, in a Queue structure (LinkedList)
	 * @throws ParseFailed Generic exception indicating program could not be parsed
	 */
	public Parser(Queue<Terminal> tokens) throws ParseFailed {
		Node root = new Node(Nonterminal.S);
		this.tokens = tokens;
		this.parse(root);
		if (!this.tokens.isEmpty()) {
			throw new ParseFailed("Unacceptable remaining input");
		}
	}

	/**
	 * Recursive function for parsing the tokens. Called implicitly by the
	 * constructor.
	 * @param node Root node
	 * @throws ParseFailed Generic parse failure exception
	 */
